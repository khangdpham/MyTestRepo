...
class ParkingLotTopo(Topo):
	...
	# Lists of switches and hosts
	switches = []
	hosts    = []
	
	# Populate switches and hosts
	for i in range(1,n+1):
		switches.append(self.addSwitch('s%d' %i))
		hosts.append(self.addHost('h%d' %i, **hconfig))	
	
	# Add receiver to first switch
...
def run_parkinglot_expt(net, n):
    ...
    senders_list = []
    for i in range(1,n+1):
	senders_list.append(net.getNodeByName('h%i' %i))
    for s in senders_list:
	s.sendCmd('iperf -c %s -p %s -t %d -i 1 -yc > %s/iperf_%s.txt' 
		  % (recvr.IP() , port , seconds, args.dir , s.name))
    progress(seconds)
    for s in senders_list:
	s.waitOutput()			
